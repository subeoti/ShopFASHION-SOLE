    ...
    public class KhuyenMaiController : ControllerBase
    {
        ...
        public List<AllViewSp> GetAllSP()
        {
            ...
            var result = _dbcontext.SanPhams
                            ...
                            .Join(_dbcontext.ChiTietSanPhams, sp => sp.sp_cl.ID, ctsp => ctsp.IDSanPham, (sp, ctsp) => new { sp_cl_lsp_ctsp = sp, chitietsps = ctsp })
                            .GroupBy(x => x.sp_cl_lsp_ctsp.sp_cl.ID)
                            .Select(group => new AllViewSp {
                                ID = group.FirstOrDefault().sp_cl_lsp_ctsp.sp_cl.ID,
                                SoLuongCTSP = group.Sum(x => x.chitietsps.ID != null ? 1 : 0),
                                Ten = group.FirstOrDefault().sp_cl_lsp_ctsp.sp_cl.Ten,
                                MoTa = group.FirstOrDefault().sp_cl_lsp_ctsp.sp_cl.MoTa,
                                TenAnh = (from anhs in _dbcontext.Anhs where @group.FirstOrDefault().sp_cl_lsp_ctsp.sp_cl.ID == anhs.IDSanPham select anhs.DuongDan).FirstOrDefault(),
        ...
        public List<AllViewSp> GetAllSPByKhuyenMai(Guid idkm)
        {
            ...
            var result = _dbcontext.SanPhams
                           ...
                           .Join(_dbcontext.ChatLieus, sp => sp.IDChatLieu, cl => cl.ID, (sp, cl) => new { sp_cl = sp, chatlieus = cl })                        
                           .Join(_dbcontext.ChiTietSanPhams, sp => sp.sp_cl.ID, ctsp => ctsp.IDSanPham, (sp, ctsp) => new { sp_cl_lsp_ctsp = sp, chitietsps = ctsp }).Where(x=>x.chitietsps.TrangThai==1)
                           .GroupBy(x => x.sp_cl_lsp_ctsp.sp_cl.ID)
                           .Select(group => new AllViewSp
                           {
                               ID = group.FirstOrDefault().sp_cl_lsp_ctsp.sp_cl.ID,
                               MaSP=group.FirstOrDefault().sp_cl_lsp_ctsp.sp_cl.Ma,
                               SoLuongCTSP = group.Sum(x => x.chitietsps.ID != null ? 1 : 0),
                               Ten = group.FirstOrDefault().sp_cl_lsp_ctsp.sp_cl.Ten,
                               MoTa = group.FirstOrDefault().sp_cl_lsp_ctsp.sp_cl.MoTa,
                               TenAnh = (from anhs in _dbcontext.Anhs where @group.FirstOrDefault().sp_cl_lsp_ctsp.sp_cl.ID == anhs.IDSanPham select anhs.DuongDan).FirstOrDefault(),
        ...
        {
            if (!_dbcontext.KhuyenMais.Any(c => c.ID == idkm)  && !_dbcontext.ChatLieus.Any(y => y.ID == idChatLieu)) throw new Exception($" khong tim thay san pham co id:{idkm},{idLoaiSP},{idChatLieu}");
            var AllCTSP = (from SP in _dbcontext.SanPhams.AsNoTracking()
                           ...
                           select new { SP, anh, chatlieu, CTSP, km });
