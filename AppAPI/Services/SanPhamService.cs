    ...
    public class SanPhamService : ISanPhamService
    {
        ...
        public async Task<ChiTietSanPhamUpdateRequest> AddSanPham(SanPhamRequest request)
        {
            try
            {
                ...
                SanPham sanPham = new SanPham() { ID = Guid.NewGuid(), Ten = request.Ten, Ma = "SP" + (max + 1), MoTa = request.MoTa, TrangThai = 1,  IDChatLieu = chatLieu.ID };
        ...
        public async Task<ChiTietSanPhamViewModelHome> GetAllChiTietSanPhamHome(Guid idSanPham)
        {
            try
            {
                ...
                var sptt = await (from sp in _context.SanPhams.Where(p => p.ID != idSanPham)
